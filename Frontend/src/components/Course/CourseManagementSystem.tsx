...
const CourseManagementSystem: FC = () => {
 ...
 const [courseData, setCourseData] = useState({
    ...
    try {
      ...
      if (response.ok) {
        ...
        const instructorCourses = currentUser?.role === 'instructor'
          ? data.filter((course: any) => course.instructor?._id === currentUser.id || course.instructor?.id === currentUser.id)
          : data;
  ...
  const addChapter = (lessonIndex: number) => {
    ...
    updatedLessons[lessonIndex].chapters = [...updatedLessons[lessonIndex].chapters, newChapter];
    setCourseData({ ...courseData, lessons: updatedLessons });
  };
  ...
  const updateChapter = (lessonIndex: number, chapterIndex: number, field: string, value: any) => {
    ...
    if (!chapter.title.trim()) {
      newErrors[`lesson_${lessonIndex}_chapter_${chapterIndex}_title`] = 'Chapter title is required';
    } else {
