...
const CourseManagementSystem = () => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            const instructorCourses = currentUser?.role === 'instructor'
              ? data.filter((course) => course.instructor?._id === currentUser.id || course.instructor?.id === currentUser.id)
              : data;
      ...
      const handleMaterialUpload = async (materialIndex, file) => {
        if (!file) return;
      ...
      const submitCourse = async (isEdit, courseData) => {
        const newErrors = {};
        if (!currentUser) {
