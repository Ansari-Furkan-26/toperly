...
// GeneralInfoForm Component
    const GeneralInfoForm = ({ courseData, setCourseData, errors, handleThumbnailUpload, uploadProgress, isEdit, currentUser, editingCourse, handleCancelEdit }) => {
      return (
    ...
    const MaterialsForm = ({ courseData, setCourseData, errors, uploadProgress, handleMaterialUpload, addMaterial, updateMaterial, removeMaterial }) => {
      return (
        <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
                      ...
                      {uploadProgress[`material_${materialIndex}`] !== undefined && (
                        <div className="mt-2">
                          ...
                          <p className="text-sm text-gray-600 mt-1">
                            {uploadProgress[`material_${materialIndex}`]}% uploaded
                          </p>
                        </div>
                      )}
                      {material.url && (
    ...
    const LessonsForm = ({ courseData, setCourseData, errors, uploadProgress, handleVideoUpload, updateLesson, addLesson, removeLesson, addChapter, updateChapter, removeChapter }) => {
      return (
        <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
...
const CourseForm = ({
      ...
      const updateChapter = (lessonIndex, chapterIndex, field, value) => {
      ...
      // Remove chapter
      const removeChapter = (lessonIndex, chapterIndex) => {
        if (!Array.isArray(courseData.lessons) || !courseData.lessons[lessonIndex] || !Array.isArray(courseData.lessons[lessonIndex].chapters)) return;
        const updatedLessons = [...courseData.lessons];
      ...
      return (
        <div className="space-y-8">
          ...
          {errors.submit && <p className="text-red-500 text-sm mt-4 max-w-4xl mx-auto">{errors.submit}</p>}
        </div>
      );
    };
