...
import { Search, Star, User, Calendar, Book, MessageSquare, ChevronDown, ChevronRight, Eye } from 'lucide-react';
...
const AdminCoursesReviews = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const reviewsResponse = await fetch('http://localhost:5000/api/reviews/all');
  ...
  useEffect(() => {
    ...
    const filtered = courses.filter(course => {
      ...
      const reviewMatch = reviews[course._id]?.some(review =>
        (review.student?.name || 'Anonymous').toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Monitor all courses and their reviews. Read-only access for administrative oversight.
          </p>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {Object.values(reviews).reduce((sum, reviewArray) => sum + reviewArray.length, 0)}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {Object.values(reviews).length > 0 
                    ? (Object.values(reviews).flat().reduce((sum, review) => sum + review.rating, 0) / 
                       Object.values(reviews).flat().length).toFixed(1)
                    : '0.0'
                  }
                </p>
              </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredData.map((course) => (
                    <React.Fragment key={course._id}> {/* Use _id as key, assuming it's the MongoDB ID */}
                      <tr className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">
                            {reviews[course._id]?.length || 0}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <div className="flex mr-2">
                              {renderStars(Math.round(getAverageRating(reviews[course._id])))}
                            </div>
                          ...
                          <td colSpan="6" className="px-6 py-4 bg-gray-50">
                            <div className="space-y-4">
                              ...
                              {reviews[course._id].map((review) => (
                                <div key={review._id} className="bg-white rounded-lg p-4 border border-gray-200"> {/* Added key with _id */}
                                  <div className="flex items-start justify-between mb-2">
                                    ...
                                    <div className="flex items-center text-xs text-gray-500">
                                      ...
                                      {new Date(review.createdAt).toLocaleDateString()} {/* Changed from review.date to createdAt */}
                                    </div>
                                  </div>
