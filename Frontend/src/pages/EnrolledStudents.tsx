...
const EnrolledStudents = () => {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const res = await axios.post('http://localhost:5000/api/students/get-mystudents', {}, {
  ...
  const calculateAnalytics = () => {
    if (studentsData.length === 0) return { totalStudents: 0, totalCourses: 0, avgProgress: 0, certificatesIssued: 0 };
    
    const totalStudents = new Set(studentsData.map(s => s.student?._id || s.student?.id)).size;
    const totalCourses = new Set(studentsData.map(s => s.course?._id || s.course?.id)).size;
    const avgProgress = (studentsData.reduce((acc, cur) => acc + (cur.progress || 0), 0) / studentsData.length).toFixed(1);
    const certificatesIssued = studentsData.filter(s => s.certificateIssued).length;
  ...
  const filteredData = studentsData.filter(entry => {
    ...
    const matchesCourse = selectedCourse === 'all' || entry.course?._id === selectedCourse;
    return matchesSearch && matchesCourse;
  });

  const uniqueCourses = [...new Set(studentsData.map(s => ({ id: s.course?._id, title: s.course?.title })))];
      ...
      <div className="bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            <div>
              <h1 className="text-3xl font-bold text-slate-900">Student Analytics Dashboard</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-1">{analytics.totalStudents}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600">Courses Taught</p>
                <p className="text-3xl font-bold text-slate-900 mt-1">{analytics.totalCourses}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-1">{analytics.avgProgress}%</p>
              </div>
              <div className="bg-amber-100 p-3 rounded-full">
                <TrendingUp className="w-6 h-6 text-amber-600" />
              </div>
            </div>
            <p className="text-xs text-amber-600 mt-4">↗ Overall completion</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-900 mt-1">{analytics.certificatesIssued}</p>
              </div>
              <div className="bg-purple-100 p-3 rounded-full">
                <Award className="w-6 h-6 text-purple-600" />
              </div>
            </div>
            <p className="text-xs text-purple-600 mt-4">↗ Achievements earned</p>
          </div>
        </div>
        ...
        {studentsData.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-600">There are no enrolled students to display at the moment.</p>
          </div>
        ) : viewMode === 'grid' ? (
                ...
                <td className="py-4 px-6">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-semibold text-sm">
                      {entry.student?.name?.split(' ').map(n => n[0]).join('').toUpperCase() || 'NA'}
                    </div>
                    <div>
                      <p className="font-medium text-slate-900">{entry.student?.name || 'Unknown'}</p>
                      <p className="text-sm text-slate-500">{entry.student?.email}</p>
                    </div>
                  </div>
                </td>
                <td className="py-4 px-6">
