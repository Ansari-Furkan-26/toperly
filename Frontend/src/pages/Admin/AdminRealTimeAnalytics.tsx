...
const AdminRealTimeAnalytics = () => {
  ...
  const [growthRate, setGrowthRate] = useState({ students: 0, instructors: 0, enrollments: 0 });
  ...
  const processAnalytics = (data: AnalyticsData) => {
    ...
    const activeStudentsCount = data.students?.filter(s => s.enrolledCourses?.length > 0).length || 0;
    setActiveStudents(activeStudentsCount);

    // Verified instructors
    const verifiedCount = data.instructors?.filter(i => i.isVerified).length || 0;
    setVerifiedInstructors(verifiedCount);

    // Average certificate marks
    if (certificatesCount > 0) {
      const totalMarks = data.certificates.reduce((sum, cert) => sum + cert.marks, 0);
  ...
  const processDailyStats = (data: AnalyticsData) => {
    ...
    const stats = last7Days.map(date => {
      ...
      const studentsOnDate = data.students?.filter(s => 
        s.createdAt && format(parseISO(s.createdAt), 'yyyy-MM-dd') === dateStr
      ).length || 0;

      const enrollmentsOnDate = data.enrollments?.filter(e => 
        e.enrolledAt && format(parseISO(e.enrolledAt), 'yyyy-MM-dd') === dateStr
      ).length || 0;
  ...
  const processTopInstructors = (data: AnalyticsData) => {
    ...
    const instructorStats = data.instructors.map(instructor => {
      ...
      const instructorReviews = data.reviews?.filter(r => 
  ...
  const processCourseAnalytics = (data: AnalyticsData) => {
    ...
    data.certificates?.forEach(cert => {
      ...
      if (courseEntries.length > 0) {
        const randomCourse = courseEntries[Math.floor(Math.random() * courseEntries.length)][1];
  ...
  const instructorPerformanceData = topInstructors.map(instructor => ({
    name: instructor.name.length > 10 ? instructor.name.substring(0, 10) + '...' : instructor.name,
  ...
  const engagementData = [
    ...
    { name: 'Verified Instructors', value: verifiedInstructors, total: totalInstructors },
    { name: 'Completed Courses', value: totalCertificates, total: totalEnrollments }
  ];
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">Live insights into platform performance and user engagement</p>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-green-500 mt-1">+{growthRate.students}% this month</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">{totalInstructors.toLocaleString()}</p>
                <p className="text-xs text-green-500 mt-1">+{growthRate.instructors}% this month</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-purple-600">{totalEnrollments.toLocaleString()}</p>
                <p className="text-xs text-green-500 mt-1">+{growthRate.enrollments}% this month</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-orange-600">{avgRating.toFixed(1)}/5.0</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <p className="text-xl font-semibold text-green-600">{totalCertificates}</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <p className="text-sm text-gray-600">Total Quizzes</p>
            <p className="text-xl font-semibold text-purple-600">{totalQuizzes}</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <p className="text-sm text-gray-600">Avg. Certificate Score</p>
            <p className="text-xl font-semibold text-orange-600">{avgCertificateMarks.toFixed(1)}%</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <ComposedChart data={dailyChartData}>
                ...
                <Tooltip />
                <Legend />
                <Area type="monotone" dataKey="students" stackId="1" stroke="#8884d8" fill="#8884d8" fillOpacity={0.6} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Top Instructor Performance</h3>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={instructorPerformanceData}>
                ...
                <Tooltip />
                <Legend />
                <Bar yAxisId="left" dataKey="courses" fill="#8884d8" name="Courses" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
              <h3 className="text-lg font-semibold text-gray-900">Daily Registration Stats</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 className="text-lg font-semibold text-gray-900">Course Analytics Overview</h3>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
