...
const MaterialsManagement = () => {
  ...
  const [editingMaterial, setEditingMaterial] = useState<{ material: Material; courseId: string } | null>(null);
  ...
  const fetchCoursesWithMaterials = async () => {
    try {
      ...
      const coursesWithMaterials = coursesData.filter(course => course.materials && course.materials.length > 0);
  ...
  const uploadFile = async (file: File): Promise<{ url: string; filename: string }> => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post("http://localhost:5000/api/url/upload", formData, {
        headers: {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm text-gray-500">Courses with Materials</p>
          ...
          <div className="space-y-4">
            {filteredCourses.length === 0 ? (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                ...
                <p className="text-gray-500 mb-6">Get started by adding your first material</p>
                <button
                  onClick={handleCreateMaterial}
                  ...
                  {expandedCourses.has(course._id) && (
                    <div className="divide-y divide-gray-200">
                      {course.materials.map(material => (
                        <div key={material.bunnyFileId} className="p-4 hover:bg-gray-50 transition-colors">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2 ml-4">
                              ...
                              <button
                                onClick={() => handleDeleteMaterial(course._id, material.bunnyFileId)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="space-y-5">
                <div>
                  ...
                  {editingMaterial && (
                    <p className="text-sm text-gray-500 mt-1">Course cannot be changed when editing</p>
                  )}
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    File Upload {editingMaterial ? '(Optional - leave empty to keep current file)' : '*'}
                  </label>
                  <input
                    type="file"
