...
const MaterialsManagement = () => {
  ...
  const handleDeleteMaterial = async (courseId: string, materialId: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `https://toperly.onrender.com/api/courses/${courseId}/materials/${materialId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const uploadFile = async (
    ...
    try {
      ...
      const response = await axios.post(
        "https://toperly.onrender.com/api/url/upload",
        formData,
        {
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingMaterial) {
        await axios.put(
          `https://toperly.onrender.com/api/courses/${formData.courseId}/materials/${editingMaterial.material.bunnyFileId}`,
          payload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        await axios.post(
          `https://toperly.onrender.com/api/courses/${formData.courseId}/materials`,
          payload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
